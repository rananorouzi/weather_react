...
class Weather extends React.Component {
    ...
    constructor() {
        ...
        this.state = {chartData : {},tempUnit : 'c'}
    }

    httpGetAsync = (theUrl, params, callback) => {
        let paramString = '';
        ...
        let urlGetString = theUrl + paramString;
    ...
    formatDateToMonthDay = (date) => {
        ...
        let d = new Date(date);
        let dateFormatted = d.getDate() + ' ' + d.toLocaleString('default', {month: 'long'});
        let hour = d.getHours();
    ...
    fetchWeatherData = (temp) => {
        ...
        this.httpGetAsync(this.weatherUrl, weatherParams, (data) => {
            ...
            if (typeof data === 'string' && data !== '') {
                ...
                if (typeof data['daily'] == 'object') {
                    ...
                    let weekTimes = data['daily']['time'];
                    let weekdailys = data['daily'];
                    ...
                    this.createDailyWeatherHTML(weekWeather);
                }
                let hourlyData = [];
                let hTimes = data['hourly']['time'];
                for (let h in hTimes) {
                    hourlyData[h] = [this.formatDateToMonthDay(hTimes[h]), data['hourly']['temperature_2m'][h]]
                }
                this.setState({ chartData: hourlyData });
                this.createCurrentWeatherHTML(currentWeather);
            } else {
    ...
    createDailyWeatherHTML =(weatherData) => {
        if (typeof weatherData == 'object') {
            let html = '<section class="daily_forecast_container"><header class="w-full text-black font-sans text-x2 font-bold self-center mb-3">Weekly Highlight</header>' +
                '<div class="grid grid-cols-3 xl:grid-cols-7 gap-2 py-2 auto-rows-fr">';
            let date, objDay, wIcon, wDesc;
            let weekday = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    ...
    createCurrentWeatherHTML = (weatherData) => {
        if (typeof weatherData == 'object') {
            let html = '<section class="current_forecast_container"><header class="w-full text-black font-sans text-x2 font-bold self-center mb-3">Current Weather</header><div class="flex">';
            let date = this.createTodayDate();
            let wIcon, wDesc;
    ...
    componentDidMount() {
        window.removeEventListener('load', this.fetchWeatherData('c'));
    }
    render() {
        return (
            <div className="container px-5 div_center">
                <div className="flex items-stretch justify-between flex-wrap div_center">
                    ...
                    <div className="right_icons temp_chose self-center justify-en flex justify-items-end  mb-3">
                        <button
                            onClick={() =>this.fetchWeatherData(this.state.tempUnit)}
                        ...
                        <button
                            onClick={() => this.fetchWeatherData('c')}
                            className="z-[2] text-white	inline-block rounded-l-lg bg-blue-500 border border-solid border-r-0 border-gray-400 px-4 pb-2 pt-2.5 text-xs font-medium leading-normal hover:bg-blue-200 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] active:bg-blue-400 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)]"
                            type="button"
                            id="temp_c"
                            data-te-ripple-init>
                            °C
                        ...
                        <button
                            onClick={() => this.fetchWeatherData('f')}
                            className="z-[2] text-slate-900	 inline-block rounded-r-lg bg-white border border-solid border-gray-400 px-5 pb-2 pt-2.5 text-xs font-medium leading-normal hover:bg-blue-200 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] active:bg-blue-400  active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)]"
                            type="button"
                            id="temp_f"
                            data-te-ripple-init>
                            °F
                ...
                <div className="overflow-hidden container lg:flex  gap-3 font-sans div_center">
                    ...
                    <div id="left_col"
                        ...
                        <div id="current" className="p-3 lg:p-5 w-full"></div>
                    </div>
                    <div id="right_col" className="bg-white border border-gray-200 rounded-lg shadow">

                        <div id="week" className="p-3 lg:p-5 mb-4"></div>
                        <div id="chart">
                            {this.createChart()}
                        </div>
                    </div>
